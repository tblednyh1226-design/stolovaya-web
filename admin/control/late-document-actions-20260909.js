// Better actions for "late iiko" alerts: open the exact movement, recalculate it, or dismiss the warning.
(function(){
  ...
  async function dismissLate(key,number){
    ...
    state.message=`Перемещение${number?` №${number}`:''} отмечаем как учтённое без пересчёта…`;
    render();
    ...
    try{
      await rpc('admin_resolve_late_document',{
        ...
        p_external_key:key,
        p_action:'dismiss',
        p_comment:'Предупреждение просмотрено администратором; принято решение не пересчитывать документ',
        p_actor:'Администратор'
      });
      state.message=`Перемещение${number?` №${number}`:''} отмечено как учтённое без пересчёта.`;
      render();
      state.busy=false;
      await load();
      state.message=`Перемещение${number?` №${number}`:''} отмечено как учтённое без пересчёта.`;
