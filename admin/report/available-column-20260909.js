...
(function(){
  ...
  matrix=function(){
    ...
    return `<section class="card section"><h2>Основной отчёт</h2><div class="wrap"><table class="matrix"><thead><tr><th class="sticky" rowspan="2">Номенклатура</th>${ps.map(p=>`<th colspan="8">${esc(p.name)}</th>`).join('')}</tr><tr>${ps.map(()=>heads.map(h=>`<th${h==='Доступно'?' class="available"':h==='Итого продано'?' class="sold"':''}>${h}</th>`).join('')).join('')}</tr></thead><tbody>${gs.map(g=>`<tr class="group-row" data-group="${encodeURIComponent(g.name)}"><td class="sticky">${state.collapsed.has(g.name)?'＋':'−'} ${esc(g.name)}</td><td colspan="${ps.length*8}"></td></tr>${state.collapsed.has(g.name)?'':g.dishes.map(d=>`<tr><td class="sticky dish">${esc(d.name)}</td>${ps.map(p=>{const r=row(d.id,p.code);return `<td>${fmt(r?.opening_qty)}</td><td title="Кухня: ${fmt(r?.kitchen_in_qty)}; принято: ${fmt(r?.internal_in_qty)}; отправлено: ${fmt(r?.internal_out_qty)}">${fmt(r?.received_qty)}</td><td class="available">${fmt(available(r))}</td><td>${fmt(r?.thawed_qty)}</td><td>${fmt(r?.leftover_qty)}</td><td>${fmt(r?.waste_qty)}</td><td>${fmt(r?.frozen_qty)}</td><td class="sold">${fmt(r?.sold_qty)}</td>`}).join('')}</tr>`).join('')}`).join('')}<tr class="total"><td class="sticky">ИТОГО</td>${ps.map(p=>{const rs=state.report.rows.filter(r=>r.point_code===p.code);return `<td>${fmt(sumRows(rs,'opening_qty'))}</td><td>${fmt(sumRows(rs,'received_qty'))}</td><td class="available">${fmt(sumAvailable(rs))}</td><td>${fmt(sumRows(rs,'thawed_qty'))}</td><td>${fmt(sumRows(rs,'leftover_qty'))}</td><td>${fmt(sumRows(rs,'waste_qty'))}</td><td>${fmt(sumRows(rs,'frozen_qty'))}</td><td class="sold">${fmt(sumRows(rs,'sold_qty'))}</td>`}).join('')}</tr></tbody></table></div></section>`;
  };
  ...
  function fixGroupTotals(){
    ...
    [...table.querySelectorAll('tbody .group-row[data-group]')].forEach(gr=>{
      ...
      tr.innerHTML=`<td class="sticky">Итого: ${esc(name)}</td>${ps.map(p=>{const rs=(state.report.rows||[]).filter(r=>r.point_code===p.code&&ids.has(r.dish_id));return `<td>${fmt(sumRows(rs,'opening_qty'))}</td><td>${fmt(sumRows(rs,'received_qty'))}</td><td class="available">${fmt(sumAvailable(rs))}</td><td>${fmt(sumRows(rs,'thawed_qty'))}</td><td>${fmt(sumRows(rs,'leftover_qty'))}</td><td>${fmt(sumRows(rs,'waste_qty'))}</td><td>${fmt(sumRows(rs,'frozen_qty'))}</td><td class="sold">${fmt(sumRows(rs,'sold_qty'))}</td>`}).join('')}`;
