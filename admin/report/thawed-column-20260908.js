// Separate "Из заморозки" from ordinary receipts in the main report.
(function(){
  ...
  exportCsv=function(){
    ...
    const head=['Группа','Номенклатура',...ps.flatMap(p=>[
      `${p.name} — Остаток на начало`,`${p.name} — Пришло`,`${p.name} — Из заморозки`,
