...
pointScreen=function(){
  ...
  return `<section class="access-card"><div class="brand-login">Столовая</div><h1>Выберите точку</h1><p>На какой точке вы сейчас работаете?</p><label style="display:block;margin:18px 0 22px;font-size:16px;font-weight:700">Дата рабочего дня <small style="display:block;font-weight:400;color:#777;margin-top:4px">Тестовый режим — доступны даты с 01.09.2026 по сегодня</small><input id="point-business-date" type="date" min="2026-09-01" max="${esc(moscowToday())}" value="${esc(day)}" style="width:100%;box-sizing:border-box;margin-top:8px;padding:14px;border:1px solid #d6d3c8;border-radius:14px;font-size:18px;background:#fff"></label><div class="point-choice-list">${state.points.map(p=>`<button type="button" data-point="${esc(p.point_code)}">${esc(p.point_name)}</button>`).join('')}</div><button id="logout" class="link-btn">Выйти</button>${state.message?`<p class="error-text" style="margin-top:16px">${esc(state.message)}</p>`:''}<p style="margin-top:18px;color:#8a8a83;font-size:13px">Сборка 07.09 • backfill 01.09+</p></section>`;
};
